...
use tokio::prelude::*;
use tokio::{
    io::{read_exact, write_all, ReadExact, WriteAll}, net::TcpStream,
};
...
impl SocksRequestResponse {
    pub fn connect(addr: &SocketAddr) -> SocksRequestResponse {
        let mut bytes = vec![v5::VERSION, v5::CMD_CONNECT, 0];
        match addr.ip() {
            IpAddr::V4(ip) => {
                bytes.push(v5::ATYP_IPV4);
                bytes.extend_from_slice(&ip.octets());
            }
            IpAddr::V6(ip) => {
                bytes.push(v5::ATYP_IPV6);
                bytes.extend_from_slice(&ip.octets());
            }
        }
        let port = [(addr.port() >> 8) as u8, addr.port() as u8];
        bytes.extend_from_slice(&port);
        SocksRequestResponse { bytes }
    }
    ...
    pub fn ipaddr(&self) -> Option<IpAddr> {
        match self.bytes[3] {
            v5::ATYP_IPV4 => Some(IpAddr::V4(Ipv4Addr::new(
                self.bytes[4],
                self.bytes[5],
                self.bytes[6],
                self.bytes[7],
            ))),
            v5::ATYP_IPV6 => Some(IpAddr::V6(Ipv6Addr::new(
...
pub fn socks_handshake(stream: TcpStream) -> SocksHandshake {
    SocksHandshake {
        ...
        state: ServerState::WaitClientAuthentication(read_exact(stream, vec![0u8; 2])),
    }
}

pub fn socks_connect_handshake(
    stream: TcpStream,
    request: SocksRequestResponse,
) -> SocksConnectHandshake {
    SocksConnectHandshake {
        request,
        state: ClientState::WaitSentAuthentication(write_all(
            stream,
            vec![v5::VERSION, 1u8, v5::METH_NO_AUTH],
        )),
        response: SocksRequestResponse {
            bytes: Vec::with_capacity(v5::MAX_REQUEST_SIZE),
        },
        ...
        loop {
            self.state = match self.state {
                WaitClientAuthentication(ref mut fut) => {
                    ...
                    ReadAuthenticationMethods(read_exact(stream, vec![0u8; buf[1] as usize]))
                }
                ReadAuthenticationMethods(ref mut fut) => {
                    ...
                    AnswerNoAuthentication(write_all(stream, vec![v5::VERSION, answer]))
                }
                AnswerNoAuthentication(ref mut fut) => {
                    ...
                    WaitClientRequest(read_exact(stream, vec![0u8; v5::MIN_REQUEST_SIZE]))
                }
                WaitClientRequest(ref mut fut) => {
                    ...
                    let dst_len = match self.request.bytes[3] {
                        v5::ATYP_IPV4 => 4,
                        v5::ATYP_IPV6 => 16,
                    ...
                    if delta == 0 {
                        let sr =
                            mem::replace(&mut self.request, SocksRequestResponse { bytes: vec![] });
                        return Ok(Async::Ready((stream, sr)));
                    }
                    WaitClientRequest(read_exact(stream, vec![0u8; delta]))
                }
            }
        }
    }
        ...
        loop {
            self.state = match self.state {
                WaitSentAuthentication(ref mut fut) => {
                    ...
                    WaitAuthenticationMethod(read_exact(stream, vec![0u8; 2]))
                }
                WaitAuthenticationMethod(ref mut fut) => {
                    ...
                    WaitSentRequest(write_all(stream, self.request.bytes.clone()))
                }
                WaitSentRequest(ref mut fut) => {
                    let (stream, _buf) = try_ready!(fut.poll());
                    WaitReply(read_exact(stream, vec![0u8; v5::MIN_REQUEST_SIZE]))
                }
                WaitReply(ref mut fut) => {
                    ...
                    if self.response.bytes[2] != self.response.bytes[2] {
                        return Err(Error::new(
                            ErrorKind::Other,
                            "Response command differs from request",
                        ));
                    };
                    let dst_len = match self.response.bytes[3] {
                        v5::ATYP_IPV4 => 4,
                        v5::ATYP_IPV6 => 16,
                    ...
                    if delta == 0 {
                        let sr = mem::replace(
                            &mut self.response,
                            SocksRequestResponse { bytes: vec![] },
                        );
                        return Ok(Async::Ready((stream, sr)));
                    }
                    WaitReply(read_exact(stream, vec![0u8; delta]))
                }
            }
        }
    }
